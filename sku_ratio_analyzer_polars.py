...
class SKURatioAnalyzer:
    ...
    def filter_valid_images(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Count SKU types per image
        image_counts = df.group_by('test_image_id').agg([
            pl.col('Prob/Ref').value_counts().alias('counts')
        ])
        
        # Explode the counts to get separate rows
        image_counts = image_counts.explode('counts')
        image_counts = image_counts.unnest('counts')
        
        # Pivot to get problem and reference counts
        image_summary = image_counts.pivot(
            values='counts',
            index='test_image_id',
            columns='Prob/Ref',
        ...
        valid_images = image_summary.filter(
            (pl.col('problem') > 0) & (pl.col('reference') > 0)
        ...
        try:
            ...
            summary_stats = pl.DataFrame({
                ...
                'Value': [
                    ...
                    self.ratio_data.select('problematic_class_name').n_unique() if self.ratio_data else 0,
                    self.ratio_data.select('reference_class_name').n_unique() if self.ratio_data else 0,
