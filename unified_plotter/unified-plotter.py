...
def check_and_install_dependencies(progress_callback=None):
    ...
    for module_name, package_name in required_packages.items():
        try:
            ...
            if progress_callback:
                progress_callback(f"Checking dependencies... ({checked_packages}/{total_packages})", 
    ...
    for i, (module_name, package_name) in enumerate(missing_packages):
        if progress_callback:
            progress_callback(f"Installing {package_name}... ({i+1}/{len(missing_packages)})", 
        ...
        if install_package(package_name):
            ...
            if progress_callback:
                progress_callback(f"Failed to install {package_name}. Please install manually.", 0)
            return False
    ...
    if 'matplotlib' in [pkg[1] for pkg in missing_packages if pkg[1] == 'matplotlib']:
        try:
            ...
            if progress_callback:
                progress_callback("Failed to re-import matplotlib after installation", 0)
            return False
...
class UnifiedScreenManager:
    ...
    def restore_welcome_screen(self, select_callback, settings_callback):
        ...
        description_label = tk.Label(self.content_frame, text=description_text, 
                                    font=tkFont.Font(family="Helvetica", size=11),
                                    justify=tk.CENTER, bg="#1a1a1a", fg="#cccccc", 
                                    pady=15, wraplength=600)
    ...
    def show_loading_screen(self):
        ...
        self.spinner_canvas = tk.Canvas(animation_frame, width=60, height=60, bg="#1a1a1a", highlightthickness=0)
        ...
        self.progress_bar = tk.Canvas(progress_frame, height=8, bg="#333333", highlightthickness=0)
        self.progress_bar.pack(fill="x")
        
        # Progress text
        self.progress_text = tk.Label(self.content_frame, text="Initializing...", 
                                     font=tkFont.Font(family="Helvetica", size=12),
                                     bg="#1a1a1a", fg="#ffffff")
        self.progress_text.pack()
        
        # Status text
        self.status_text = tk.Label(self.content_frame, text="Loading dependencies and preparing interface...", 
        ...
        # Tips title
        tips_title = tk.Label(tips_frame, text="💡 Quick Tips", 
        ...
        for tip in tips_content:
            tip_label = tk.Label(tips_frame, text=tip, 
                               font=tkFont.Font(family="Helvetica", size=10),
        ...
        # Features title
        features_title = tk.Label(features_frame, text="🚀 Professional Features", 
                                 font=tkFont.Font(family="Helvetica", size=14, weight="bold"),
                                 bg="#1a1a1a", fg="#00ff88")
        ...
        try:
            ...
            version_text = tk.Label(self.content_frame, text=get_version_string(), 
                                   font=tkFont.Font(family="Helvetica", size=9),
                                   bg="#1a1a1a", fg="#555555")
        except ImportError:
            # Fallback if version module is not available
            version_text = tk.Label(self.content_frame, text="Version 2.1.0 | Professional Edition", 
                                   font=tkFont.Font(family="Helvetica", size=9),
                                   bg="#1a1a1a", fg="#555555")
    ...
    def show_progress_screen(self, message, progress=0, total=100):
        ...
        # Title
        title_label = tk.Label(self.content_frame, text="Processing Your Data", 
                              font=tkFont.Font(family="Helvetica", size=16, weight="bold"),
                              bg="#1a1a1a", fg="#ffffff")
        title_label.pack(pady=(0, 20))
        
        # Status message
        self.status_text = tk.Label(self.content_frame, text=message, 
                                   font=tkFont.Font(family="Helvetica", size=12),
                                   bg="#1a1a1a", fg="#cccccc", wraplength=600)
        ...
        self.progress_bar = tk.Canvas(progress_frame, height=12, bg="#333333", highlightthickness=0)
        self.progress_bar.pack(fill="x")
        
        # Progress text
        self.progress_text = tk.Label(self.content_frame, text=f"Progress: {progress}/{total} ({int(progress/total*100)}%)", 
                                     font=tkFont.Font(family="Helvetica", size=10),
                                     bg="#1a1a1a", fg="#888888")
        self.progress_text.pack()
        ...
        self.progress_bar.create_rectangle(0, 0, bar_width, 12, fill="#00ff88", outline="")
    ...
    def show_error_screen(self, title, message, button_text="Home", button_callback=None):
        ...
        title_label = tk.Label(self.content_frame, text=f"❌ {title}", 
                              font=tkFont.Font(family="Helvetica", size=18, weight="bold"),
                              bg="#1a1a1a", fg="#ff4444")
        title_label.pack(pady=(0, 20))
        
        # Error message
        error_text = tk.Label(self.content_frame, text=message, 
                             font=tkFont.Font(family="Helvetica", size=11),
                             bg="#1a1a1a", fg="#cccccc", justify=tk.LEFT, 
                             wraplength=600)
        ...
        # Suggestions title
        suggestions_title = tk.Label(suggestions_frame, text="💡 What you can do:", 
        ...
        for suggestion in suggestions_content:
            suggestion_label = tk.Label(suggestions_frame, text=suggestion, 
                                      font=tkFont.Font(family="Helvetica", size=10),
                                      bg="#1a1a1a", fg="#cccccc", anchor="w")
    ...
    def animate_spinner(self):
        ...
        # Draw spinner arc
        self.spinner_canvas.create_arc(center_x - radius, center_y - radius, 
                                      center_x + radius, center_y + radius,
                                      start=self.spinner_angle, extent=270, 
                                      outline="#00ff88", width=3, style="arc")
        
        # Draw center dot
        self.spinner_canvas.create_oval(center_x - 3, center_y - 3, 
    ...
    def update_loading_progress(self):
        ...
        if self.current_step < len(self.loading_steps):
            ...
            self.progress_bar.create_rectangle(0, 0, bar_width, 8, fill="#00ff88", outline="")
    ...
    def update_progress(self, message, progress, total=100):
        ...
        if self.current_mode == "progress" and self.status_text and self.progress_text and self.progress_bar:
            try:
                # Check if widgets still exist before accessing them
                if hasattr(self.status_text, 'winfo_exists') and self.status_text.winfo_exists():
                    self.status_text.config(text=message)
                if hasattr(self.progress_text, 'winfo_exists') and self.progress_text.winfo_exists():
                    self.progress_text.config(text=f"Progress: {progress}/{total} ({int(progress/total*100)}%)")
...
def show_loading_screen():
    ...
    screen_manager.create_unified_window("Unified Plotter | Professional Bounding Box Visualization", show_title_bar=False)
...
def show_welcome_screen_and_get_filepath():
    ...
    def show_settings_page():
        ...
        try:
            ...
            def calculate_performance_score(profile):
                ...
                score += min(profile['cpu_cores'] / 8, 1) * 30  # CPU: 30 points (8 cores = 100%)
            ...
            device_frame = tk.LabelFrame(cards_inner, text="📱 Device Information", 
            ...
            profile_frame = tk.LabelFrame(cards_inner, text="🚀 Performance Profile", 
                                        font=tkFont.Font(family="Helvetica", size=15, weight="bold"),
                                        bg="#2a2a2a", fg="#ffffff", padx=15, pady=15)
            ...
            features_frame = tk.LabelFrame(cards_inner, text="🎨 Feature Toggles", 
                                         font=tkFont.Font(family="Helvetica", size=15, weight="bold"),
                                         bg="#2a2a2a", fg="#ffffff", padx=15, pady=15)
            ...
            feature_checkboxes['high_quality'] = create_feature_checkbox(features_frame, "High-Quality Thumbnails", 
                                                                       settings['high_quality_thumbnails'], 
                                                                       "Recommended for your device")
            feature_checkboxes['real_time_hover'] = create_feature_checkbox(features_frame, "Real-Time Hover", 
                                                                          settings['real_time_hover'], 
                                                                          "Smooth hover interactions")
            feature_checkboxes['smooth_animations'] = create_feature_checkbox(features_frame, "Smooth Animations", 
            ...
            memory_frame = tk.LabelFrame(cards_inner, text="💾 Memory Management", 
                                       font=tkFont.Font(family="Helvetica", size=15, weight="bold"),
                                       bg="#2a2a2a", fg="#ffffff", padx=15, pady=15)
    ...
    def apply_performance_profile(profile_name):
        """Apply predefined performance profile settings"""
        if profile_name == 'high':
            settings['show_background_images'].set(False)  # Disabled by default for all profiles
            settings['high_quality_thumbnails'].set(True)
            settings['real_time_hover'].set(True)
            settings['smooth_animations'].set(True)
            settings['anti_aliasing'].set(True)
            ...
            settings['image_caching'].set(True)
            settings['aggressive_cleanup'].set(False)
            settings['disable_background_image_button'].set(True)  # Button disabled by default for all profiles
        elif profile_name == 'balanced':
            settings['show_background_images'].set(False)  # Disabled by default for all profiles
            settings['high_quality_thumbnails'].set(True)
            settings['real_time_hover'].set(True)
    ...
    def show_settings_page():
        ...
        # Settings title
        settings_title = tk.Label(settings_frame, text="⚙️ Performance Settings", font=title_font, bg="#1a1a1a", fg="#ffffff", pady=5)
        ...
        device_frame = tk.LabelFrame(scrollable_frame, text="📱 Device Information", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        ...
        device_label = tk.Label(device_frame, text=device_info, font=body_font, bg="#ffffff", fg="#555555")
        device_label.pack(pady=2)
        
        score_info = f"Performance Score: {performance_score}/100"
        score_label = tk.Label(device_frame, text=score_info, font=body_font, bg="#ffffff", fg="#555555")
        score_label.pack(pady=2)
        
        suggestion_info = f"Suggested Mode: {suggested_text}"
        suggestion_label = tk.Label(device_frame, text=suggestion_info, font=body_font, bg="#ffffff", fg="#28a745")
        suggestion_label.pack(pady=2)
        
        # Performance profile section
        profile_frame = tk.LabelFrame(scrollable_frame, text="🚀 Performance Profile", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        ...
        tk.Radiobutton(profile_frame, text="High Performance (All features)", variable=profile_var, value="high", 
                      command=on_profile_change, font=body_font, bg="#ffffff", fg="#333333").pack(anchor="w", pady=2)
        tk.Radiobutton(profile_frame, text="Balanced (Recommended)", variable=profile_var, value="balanced", 
                      command=on_profile_change, font=body_font, bg="#ffffff", fg="#333333").pack(anchor="w", pady=2)
        tk.Radiobutton(profile_frame, text="Low-End Optimized", variable=profile_var, value="low", 
                      command=on_profile_change, font=body_font, bg="#ffffff", fg="#333333").pack(anchor="w", pady=2)
        tk.Radiobutton(profile_frame, text="Custom", variable=profile_var, value="custom", 
                      command=on_profile_change, font=body_font, bg="#ffffff", fg="#333333").pack(anchor="w", pady=2)
        
        # Feature toggles section
        features_frame = tk.LabelFrame(scrollable_frame, text="🎨 Feature Toggles", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        ...
        def create_feature_checkbox(parent, text, setting_var, description=""):
            ...
            cb = tk.Checkbutton(frame, text=text, variable=setting_var, font=body_font, bg="#ffffff", fg="#333333")
            cb.pack(side="left")
            
            if description:
                desc_label = tk.Label(frame, text=description, font=tkFont.Font(family="Helvetica", size=9), 
                                    bg="#ffffff", fg="#888888")
        ...
        feature_checkboxes['high_quality'] = create_feature_checkbox(features_frame, "High-Quality Thumbnails", 
                                                                   settings['high_quality_thumbnails'], 
                                                                   "Recommended for your device")
        feature_checkboxes['real_time'] = create_feature_checkbox(features_frame, "Real-time Hover Labels", 
                                                                settings['real_time_hover'])
        feature_checkboxes['smooth'] = create_feature_checkbox(features_frame, "Smooth Animations", 
                                                             settings['smooth_animations'], 
        ...
        additional_frame = tk.LabelFrame(scrollable_frame, text="🔧 Additional Settings", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        additional_frame.pack(fill="x", padx=5, pady=5)
        
        feature_checkboxes['disable_bg_button'] = create_feature_checkbox(additional_frame, "Disable Background Image Button", 
                                                                        settings['disable_background_image_button'], 
                                                                        "Enabled by default - removes button from UI")
        feature_checkboxes['save_plots'] = create_feature_checkbox(additional_frame, "Save Plots on Program Close", 
                                                                 settings['save_plots_on_close'], 
                                                                 "Automatically save plots when closing")
        
        # Memory management section
        memory_frame = tk.LabelFrame(scrollable_frame, text="💾 Memory Management", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        ...
        thumbnail_frame = tk.LabelFrame(scrollable_frame, text="🖼️ Thumbnail Settings", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        ...
        width_label = tk.Label(width_frame, text="Thumbnail Width (% of figure):", font=body_font, bg="#ffffff", fg="#333333")
        width_label.pack(side="left")
        
        width_var = tk.StringVar(value="5.0")
        width_entry = tk.Entry(width_frame, textvariable=width_var, font=body_font, width=8)
        ...
        padding_label = tk.Label(padding_frame, text="Thumbnail Padding (% of figure):", font=body_font, bg="#ffffff", fg="#333333")
        padding_label.pack(side="left")
        
        padding_var = tk.StringVar(value="0.8")
        padding_entry = tk.Entry(padding_frame, textvariable=padding_var, font=body_font, width=8)
        padding_entry.pack(side="left", padx=(10, 0))
        
        # Logging settings section
        logging_frame = tk.LabelFrame(scrollable_frame, text="📝 Logging & Debugging", font=body_font, bg="#ffffff", fg="#333333", padx=10, pady=5)
        ...
        retention_label = tk.Label(retention_frame, text="Log Retention:", font=body_font, bg="#ffffff", fg="#333333")
        retention_label.pack(side="left")
        
        retention_var = settings['log_retention']
        retention_menu = tk.OptionMenu(retention_frame, retention_var, 'Daily', 'Weekly', 'Monthly', 'Yearly')
        ...
        debug_checkbox = tk.Checkbutton(logging_frame, text="Enable Debug Logging", variable=debug_var, 
                                       font=body_font, bg="#ffffff", fg="#333333")
        ...
        download_logs_btn = tk.Button(log_buttons_frame, text="📥 Download Logs", 
                                     command=download_logs, font=body_font, 
                                     bg="#007bff", fg="black", 
                                     activebackground="#0056b3", activeforeground="black",
                                     relief=tk.FLAT, borderwidth=1, padx=15, pady=5)
        download_logs_btn.pack(side="left", padx=(0, 10))
        
        delete_logs_btn = tk.Button(log_buttons_frame, text="🗑️ Delete All Logs", 
                                   command=delete_logs, font=body_font, 
                                   bg="#dc3545", fg="black", 
        ...
        def save_settings():
            ...
            try:
                ...
                if thumb_padding < 0.005 or thumb_padding > 0.05:
                    print("⚠ Thumbnail padding out of range (0.5%-5%), using default 1%")
                    thumb_padding = 0.01
            ...
            global_settings = {
                ...
                'disable_background_image_button': settings['disable_background_image_button'].get(),
        ...
        save_btn = tk.Button(buttons_frame, text="Save Settings", command=save_settings, 
                            font=button_font, bg="#28a745", fg="black", 
                            activebackground="#218838", activeforeground="black",
                            relief=tk.FLAT, borderwidth=0, padx=20, pady=8, cursor="")
        save_btn.pack(side="left", padx=(0, 10))
        
        cancel_btn = tk.Button(buttons_frame, text="Cancel", command=cancel_settings, 
                              font=button_font, bg="#6c757d", fg="black", 
                              activebackground="#5a6268", activeforeground="black",
                              relief=tk.FLAT, borderwidth=0, padx=20, pady=8, cursor="")
    ...
    def show_welcome_page():
        ...
        welcome_label = tk.Label(main_frame, text="Unified Plotter", font=title_font, bg="#1a1a1a", fg="#ffffff", pady=15)
        welcome_label.pack(pady=(0, 15))
        
        # Subtitle
        subtitle_label = tk.Label(main_frame, text="Professional Edition", font=tkFont.Font(family="Helvetica", size=16), bg="#1a1a1a", fg="#00ff88", pady=8)
        ...
        description_label = tk.Label(main_frame, text=description_text, font=body_font, justify=tk.CENTER, bg="#1a1a1a", fg="#cccccc", pady=15, wraplength=600)
        description_label.pack()
    ...
    main_frame = tk.Frame(root, bg="#1a1a1a", padx=30, pady=30, relief=tk.RAISED, borderwidth=2)
...
def cleanup_old_logs():
    ...
    try:
        ...
        if deleted_count > 0:
            logging.info(f"Cleaned up {deleted_count} old log files (older than {retention_setting} - {retention_days} days)")
    ...
    try:
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            for log in logs:
                ...
                f.write(f"  Modified: {log['modified'].strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        
        messagebox.showinfo("Logs Downloaded", 
...
def delete_logs():
    ...
    try:
        ...
        messagebox.showinfo("Logs Deleted", f"Successfully deleted {deleted_count} log files.")
    ...
    for idx_row, row in df_selected.iterrows():
        if row['x_min'] <= x <= row['x_max'] and row['y_min'] <= y <= row['y_max']:
            ...
            if label_lines:
                ...
                print(f"  📏 Plot limits: X({xlim[0]:.1f}, {xlim[1]:.1f}), Y({ylim[0]:.1f}, {ylim[1]:.1f})")
                print(f"  📍 Text position: ({adjusted_x:.1f}, {adjusted_y:.1f})")
                print(f"  🎯 Mouse position: ({x:.1f}, {y:.1f})")
                
                if state.hover_text is None:
                    try:
                        print(f"  🎨 Creating new hover text at ({adjusted_x:.1f}, {adjusted_y:.1f})")
                        # Restore original label format with white box and blue text
                        state.hover_text = main_ax.text(adjusted_x, adjusted_y, hover_text, 
                                                      color='blue', fontsize=10, va='bottom', ha='left', 
                                                      bbox=dict(facecolor='white', alpha=0.98, edgecolor='black', boxstyle='round,pad=0.5'),
                                                      zorder=10000)  # Extremely high z-order to appear above everything
                        print(f"  ✅ Hover text created: {state.hover_text}")
                        print(f"  🔍 Text properties: visible={state.hover_text.get_visible()}, alpha={state.hover_text.get_alpha()}")
                    except (NotImplementedError, ValueError) as e:
                        print(f"  ❌ Error creating hover text: {e}")
                        pass
                else:
                    try:
                        print(f"  🔄 Updating existing hover text at ({adjusted_x:.1f}, {adjusted_y:.1f})")
            ...
            for _, row in df_selected.iterrows():
                ...
                if marked_value and marked_value.lower() != 'nan' and marked_value.lower() != '':
                    ...
                    if marked_value.lower() == 'yes':
                        ...
                        marker_size = 10
                        # Display as X marker with high z-order
                        ax.plot(x, y, marker='x', color=marker_color, markersize=marker_size, mew=2, zorder=10)
                    else:
                        display_value = marked_value
                        marker_color = 'purple'  # Different color for other existing marks
                        # Display as text (no X marker) with high z-order
                        ax.text(x, y, display_value, color=marker_color, fontsize=10, 
                               ha='center', va='center', weight='light', zorder=10)
...
def show_save_confirmation():
    ...
    try:
        ...
        logo_text = tk.Label(logo_frame, text="Unified Plotter", 
                            font=tkFont.Font(family="Helvetica", size=24, weight="bold"),
                            bg="#1a1a1a", fg="#ffffff")
        logo_text.pack()
        
        subtitle_text = tk.Label(logo_frame, text="Professional Bounding Box Visualization", 
                               font=tkFont.Font(family="Helvetica", size=11),
                               bg="#1a1a1a", fg="#888888")
        subtitle_text.pack(pady=(5, 0))
        ...
        question_text = tk.Label(question_frame, text="💾 Save Options Before Closing?", 
                               font=tkFont.Font(family="Helvetica", size=18, weight="bold"),
                               bg="#1a1a1a", fg="#00ff88")
        question_text.pack()
        
        # Description
        desc_text = tk.Label(question_frame, 
                           text="Choose how you want to save your work before closing the application:",
                           font=tkFont.Font(family="Helvetica", size=12),
                           bg="#1a1a1a", fg="#cccccc", justify=tk.CENTER)
        ...
        yes_button = tk.Button(button_frame, text="💾 Yes - save all plots and files", 
                              command=on_yes,
                              font=tkFont.Font(family="Helvetica", size=12, weight="bold"),
                              bg="#00ff88", fg="#1a1a1a", 
                              activebackground="#00cc6a", activeforeground="#1a1a1a",
        ...
        no_button = tk.Button(button_frame, text="📄 No - save files only", 
                             command=on_no,
                             font=tkFont.Font(family="Helvetica", size=12, weight="bold"),
                             bg="#ffaa00", fg="#1a1a1a", 
                             activebackground="#cc8800", activeforeground="#1a1a1a",
                             relief=tk.FLAT, padx=25, pady=12, cursor="hand2")
        no_button.pack(fill="x", pady=(0, 10))
        
        # Cancel button - Return to plot screen
        cancel_button = tk.Button(button_frame, text="↩️ Cancel", 
                                 command=on_cancel,
                                 font=tkFont.Font(family="Helvetica", size=12, weight="bold"),
                                 bg="#666666", fg="#1a1a1a", 
                                 activebackground="#555555", activeforeground="#1a1a1a",
                                 relief=tk.FLAT, padx=25, pady=12, cursor="hand2")
...
def show_processing_progress(message, progress=0, total=100):
    ...
    screen_manager.create_unified_window("Unified Plotter | Processing Data", show_title_bar=True)
    screen_manager.show_progress_screen(message, progress, total)
    return screen_manager, screen_manager.status_text, screen_manager.progress_text, screen_manager.progress_bar
...
File: {os.path.basename(file_path)}"""
        ...
        screen_manager.create_unified_window("Unified Plotter | Invalid CSV File", show_title_bar=True)
        screen_manager.show_error_screen("Invalid CSV File", error_message, "Home", go_home)
        screen_manager.run()
        return False
...
def process_csv_file(file_path):
    ...
    progress_manager, status_label, progress_text, progress_bar = show_processing_progress("Loading CSV file...", 0, 100)
...
File: {os.path.basename(file_path)}"""
        ...
        screen_manager.create_unified_window("Unified Plotter | Error Loading CSV", show_title_bar=True)
        screen_manager.show_error_screen("Error Loading CSV", error_message, "Home", go_home)
        screen_manager.run()
        return False
    ...
    if 'marked' in df.columns:
        ...
        progress_manager.update_progress(f"Generating thumbnails (0/{len(image_ids)})...", 80, 100)
    
    logger.info("Starting plotting interface creation...")
    # Generate thumbnails and create the main plotting interface
    create_plotting_interface_with_progress(progress_manager)
    ...
    try:
        ...
        if matplotlib.get_backend() in ['Agg']:
            print("⚠ Non-interactive backend detected. Plot window cannot be displayed.")
    ...
    if global_settings.get('progressive_loading', False):
        ...
        for i, img_id in enumerate(image_ids):
            ...
            if i < 5:
                ...
                root.after(100 * i, lambda idx=i, img=img_id: load_thumbnail_progressive(img, idx))
        
        print(f"✓ Created {len(thumbnails)} placeholder thumbnails (progressive loading enabled)")
    else:
        # Standard loading for high-end devices with progress feedback
        for i, img_id in enumerate(image_ids):
            try:
                # Update progress
                progress_percent = 80 + int((i / total_images) * 15)  # 80-95% for thumbnails
                progress_manager.update_progress(f"Generating thumbnails ({i+1}/{total_images})...", progress_percent, 100)
...
def create_main_plot_interface():
    ...
    try:
        ...
        # Method 3: Try setting figure properties
            ...
            try:
                ...
                if hasattr(tk_widget, 'master') and hasattr(tk_widget.master, 'title'):
                    tk_widget.master.title("Unified Plotter - Professional Bounding Box Visualization")
        ...
        try:
            fig.suptitle("Unified Plotter - Professional Bounding Box Visualization", fontsize=16, y=0.95)
    ...
    try:
        ...
        main_ax.set_zorder(1000)  # Set high z-order for main plot so labels can appear above
    ...
    for i in range(len(image_ids)):
        try:
            ...
            ax.set_title(f'{image_ids[i]}', fontsize=8, y=-0.35)  # Consistent y offset for uniform padding
            ax.set_xticks([])
            ax.set_yticks([])
    ...
    try:
        # Create help text
        help_text = thumb_container_ax.text(0.5, -0.3, 'Use ←/→ or A/D to navigate, R/S/L/F/B/O for actions, H/?/F1 for professional help', 
                                           ha='center', va='center', fontsize=9, color='gray', alpha=0.7)
        
        # Create help button to the right of the help text
        help_button_ax = fig.add_axes([0.95, 0.02, 0.03, 0.03])  # Position at right side
    ...
    try:
        ...
        if website_url and website_url.strip() != '':
            ...
            website_button_ax = fig.add_axes([0.91, 0.02, 0.03, 0.03])  # Position to the left of help button
            website_button_ax.set_zorder(100)
            
            # Create website button with reliable icon
            try:
                # Try professional text first (most reliable)
                btn_website = Button(website_button_ax, 'WEB', color='white', hovercolor='lightgray')
                print("✓ Website button created with 'WEB' text")
            except Exception as e:
                print(f"⚠ Error creating button with 'WEB' text: {e}")
                try:
                    # Fallback to simple text
                    btn_website = Button(website_button_ax, 'LINK', color='white', hovercolor='lightgray')
                    print("✓ Website button created with 'LINK' text")
                except Exception as e2:
                    print(f"⚠ Error creating button with 'LINK' text: {e2}")
                    # Last resort
                    btn_website = Button(website_button_ax, 'WWW', color='white', hovercolor='lightgray')
    ...
    print("✓ Keyboard navigation help text, help button, and website button creation completed")
...
def create_control_widgets():
    ...
    try:
        ...
        ax_toggle_labels.set_zorder(100)  # Set low z-order so labels appear above buttons
    ...
    if any(state.image_url for state in annotation_states.values()):
        try:
            ...
            ax_open_image.set_zorder(100)  # Set low z-order so labels appear above buttons
        ...
        if not global_settings.get('disable_background_image_button', True):
            try:
                ax_show_bg = fig.add_axes([left + 0.02*width, bottom - 0.11*height, 0.9*width, 0.07*height])
                ax_show_bg.set_zorder(100)  # Set low z-order so labels appear above buttons
...
def create_interactive_help_content():
    ...
    # Title
    title_text = help_container.text(0.5, 0.95, 'KEYBOARD SHORTCUTS REFERENCE', 
                                    ha='center', va='center', fontsize=14, 
                                    fontweight='bold', color='#2E86AB',
                                    transform=help_container.transAxes)
    
    # Navigation section
    nav_title = help_container.text(0.1, 0.85, 'NAVIGATION', 
    ...
    for i, (shortcut, description) in enumerate(nav_items):
        ...
        shortcut_text = help_container.text(0.15, y_pos, shortcut, 
                                          fontsize=10, color='#0066cc', 
                                          fontweight='bold', style='italic',
                                          transform=help_container.transAxes)
        shortcut_text.set_bbox(dict(boxstyle='round,pad=0.2', facecolor='#f0f8ff', 
                                   edgecolor='#0066cc', alpha=0.8))
        nav_links.append(('nav', shortcut, shortcut_text))
        
        # Description text
        help_container.text(0.45, y_pos, description, 
                           fontsize=10, color='#333333',
                           transform=help_container.transAxes)
    
    # Actions section
    action_title = help_container.text(0.1, 0.55, 'ACTIONS', 
                                      fontsize=12, fontweight='bold', color='#333333',
                                      transform=help_container.transAxes)
    ...
    native_title = help_container.text(0.1, 0.25, 'NATIVE OS SHORTCUTS', 
                                      fontsize=12, fontweight='bold', color='#333333',
                                      transform=help_container.transAxes)
    ...
    resources_title = help_container.text(0.1, 0.05, 'ADDITIONAL RESOURCES', 
                                         fontsize=12, fontweight='bold', color='#333333',
                                         transform=help_container.transAxes)
    
    # Website link (clickable)
    website_text = help_container.text(0.15, 0.02, '🌐 Visit Developer Website', 
                                      fontsize=10, color='#0066cc', 
                                      fontweight='bold', style='italic',
                                      transform=help_container.transAxes)
    website_text.set_bbox(dict(boxstyle='round,pad=0.2', facecolor='#f0f8ff', 
                               edgecolor='#0066cc', alpha=0.8))
    
    # Store all links for click handling
    all_links = nav_links + action_links + native_links + [('website', 'https://raghavendrapratap.com/', website_text)]
    ...
    try:
        ...
        if link_type == 'nav':
            if '←' in shortcut or '→' in shortcut or 'A' in shortcut or 'D' in shortcut:
                show_help_tooltip("Use these keys to navigate between images in your dataset")
                ...
                show_help_tooltip("Jump 10 images forward or backward for faster navigation")
            elif '1-9' in shortcut:
                show_help_tooltip("Quick jump to specific image positions (1st through 9th)")
        
        elif link_type == 'action':
            if shortcut == 'R':
                show_help_tooltip("Reset the annotation counter back to 1")
            elif shortcut == 'S':
                show_help_tooltip("Save all your annotations and updated data to CSV files")
            elif shortcut == 'L':
                show_help_tooltip("Toggle hover labels on/off for bounding boxes")
            elif shortcut == 'F':
                show_help_tooltip("Flip the Y-axis orientation (useful for different coordinate systems)")
            elif shortcut == 'B':
                show_help_tooltip("Toggle background image display (if enabled in settings)")
            ...
            if 'Ctrl+Z' in shortcut or 'Cmd+Z' in shortcut:
                ...
                show_help_tooltip("Redo a previously undone annotation (standard OS shortcut)")
...
def show_help_tooltip(message):
    ...
    try:
        ...
        # Add tooltip text
        tooltip_ax.text(0.5, 0.5, message, ha='center', va='center', 
    ...
    try:
        ...
        try:
            ...
            if icon_type in fa_icon_map:
                try:
                    btn = Button(ax, fa_icon_map[icon_type], color='white', hovercolor='lightgray')
                    print(f"✓ Button created with FontAwesome icon: {fa_icon_map[icon_type]}")
                    icon_created = True
                    return btn
                except Exception as e:
        ...
        if not icon_created:
            try:
                ...
                if icon_type in mi_icon_map:
                    try:
                        btn = Button(ax, mi_icon_map[icon_type], color='white', hovercolor='lightgray')
                        print(f"✓ Button created with Material icon: {mi_icon_map[icon_type]}")
                        icon_created = True
                        return btn
                    except Exception as e:
...
╚══════════════════════════════════════════════════════════════════════════════╝"""
        ...
        if not help_text_box:
            try:
                ...
                help_text_box = fig.text(0.5, 0.5, help_content, 
                                        ha='center', va='center', fontsize=9,
                                        fontfamily='monospace',  # Use monospace for table alignment
                                        bbox=dict(facecolor='white', alpha=0.98, 
                                                 edgecolor='#2E86AB', boxstyle='round,pad=1.0',
                                                 linewidth=2),
                                        transform=fig.transFigure, zorder=10000)
        ...
        if help_text_box and hasattr(help_text_box, 'get_visible'):
            ...
            if not is_visible:
                print("⚠ Warning: Help text should be visible but isn't - forcing visibility")
                help_text_box.set_visible(True)
                fig.canvas.draw()
...
def connect_events():
    ...
    if btn_website:
        ...
        print("ℹ Website button not available for connection - button was not created (no URL configured)")
...
def return_to_welcome(event=None):
    """Return to the welcome screen from the plot screen"""
    global _close_operation_in_progress
    ...
    try:
        ...
        if save_option == "save_all":
            # Create output directory with timestamp in the CSV file's directory
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            if output_dir is None:
                # Fallback to current directory if output_dir is not set
                output_dir = os.path.join(os.getcwd(), f'plots_{timestamp}')
            else:
                # output_dir is already the CSV file's directory, just add the timestamped folder
        ...
        # Save annotation CSV files only if user chose to save something
        if save_option in ["save_all", "save_annotations_only"]:
            logger.info("Saving annotation data...")
            save_annotations()
        
        # Now close the matplotlib figure
        plt.close('all')
        print("✓ Returning to welcome screen...")
        
        # The main loop will handle returning to welcome screen
...
if __name__ == "__main__":
    try:
        ...
        if not check_and_install_dependencies(update_progress):
            ...
            try:
                screen_manager.show_error_screen("Dependency Error", 
