...
def show_welcome_screen_and_get_filepath():
    ...
    root = tk.Tk()
    root.title("Bounding Box Plotter")
    root.configure(bg="#f0f0f0")  # Set a light gray background

    # Store file path in a list to be mutable by inner functions
    file_path_holder = [""]
    ...
    settings = {
        'performance_mode': tk.StringVar(value='balanced'),
        'show_background_images': tk.BooleanVar(value=False),  # Background images disabled by default
        'high_quality_thumbnails': tk.BooleanVar(value=True),
        'real_time_hover': tk.BooleanVar(value=True),
        'smooth_animations': tk.BooleanVar(value=True),
        ...
        'disable_background_image_button': tk.BooleanVar(value=True),  # Button disabled by default
        'save_plots_on_close': tk.BooleanVar(value=True),  # Default enabled
        'log_retention': tk.StringVar(value='Monthly'),  # Default to monthly
        'enable_debug_logging': tk.BooleanVar(value=False)  # Default disabled
    }
    ...
    def show_settings_page():
        ...
        # Settings title
        settings_title = tk.Label(settings_frame, text="⚙️ Performance Settings", font=title_font, bg="#ffffff", fg="#333333", pady=5)
        settings_title.pack(pady=(0, 10))
        
        # Create scrollable frame for settings
        canvas = tk.Canvas(settings_frame, bg="#ffffff", highlightthickness=0)
    ...
    def show_welcome_page():
        ...
        # Description Text
        description_text = (
            "Welcome! This tool helps you visualize and annotate bounding box data directly from your CSV file.\n\n\n"
            "Please note!\n"
            "Your CSV must include columns for `image_id`, `x_min`, `x_max`, `y_min`, `y_max`\n"
            "Optionally you can add `label_*` or image URL columns.\n\n\n"
            "Click the button below to get started.\n"
            "⬇"
        )
        description_label = tk.Label(main_frame, text=description_text, font=body_font, justify=tk.CENTER, bg="#ffffff", fg="#555555", pady=10)
        ...
        # Select File Button with enhanced styling (centered)
        ...
        select_button.pack(pady=(0, 20))  # Center the main button with bottom margin
        
        # Bottom row for settings and exit buttons
        bottom_buttons_frame = tk.Frame(main_frame, bg="#ffffff")
        bottom_buttons_frame.pack(side="bottom", fill="x", padx=10, pady=(0, 10))
        
        # Settings Button (bottom left) - Enhanced styling
        ...
        # Exit Button (bottom right) - Enhanced styling
        ...
        # Enhanced hover effects with smooth transitions and visual feedback
        def on_enter_select(e):
            select_button['background'] = '#4f46e5'  # Darker indigo on hover
            select_button['borderwidth'] = 3  # Thicker border on hover
            select_button['relief'] = tk.RAISED  # 3D effect on hover
            select_button['fg'] = '#000000'  # Ensure black text
        
        def on_leave_select(e):
            select_button['background'] = '#6366f1'  # Return to original indigo
            select_button['borderwidth'] = 2  # Return to normal border
            select_button['relief'] = tk.FLAT  # Return to flat relief
            select_button['fg'] = '#000000'  # Ensure black text
        
        def on_enter_settings(e):
            settings_button['background'] = '#059669'  # Darker emerald on hover
            settings_button['borderwidth'] = 3  # Thicker border on hover
            settings_button['relief'] = tk.RAISED  # 3D effect on hover
            settings_button['fg'] = '#000000'  # Ensure black text
        
        def on_leave_settings(e):
            settings_button['background'] = '#10b981'  # Return to original emerald
            settings_button['borderwidth'] = 2  # Return to normal border
            settings_button['relief'] = tk.FLAT  # Return to flat relief
            settings_button['fg'] = '#000000'  # Ensure black text
        
        def on_enter_exit(e):
            exit_button['background'] = '#d97706'  # Darker amber on hover
            exit_button['borderwidth'] = 3  # Thicker border on hover
            exit_button['relief'] = tk.RAISED  # 3D effect on hover
            exit_button['fg'] = '#000000'  # Ensure black text
        
        def on_leave_exit(e):
            exit_button['background'] = '#f59e0b'  # Return to original amber
            exit_button['borderwidth'] = 2  # Return to normal border
    ...
    # Create a main frame with enhanced styling
    main_frame = tk.Frame(root, bg="#ffffff", padx=20, pady=20, relief=tk.RAISED, borderwidth=3)
...
def on_close(event=None):
    ...
    logger.info("Program closing, saving all data...")
    print("Saving all data before closing...")
    
    # Check if plots should be saved based on settings
    if global_settings.get('save_plots_on_close', True):
        # Save annotated plots
        logger.info("Saving annotated plots...")
        save_all_annotated_plots()
        print("✓ Plots saved successfully")
    else:
        logger.info("Plot saving disabled by settings")
        print("ℹ Plot saving disabled by settings")
    
    # Save annotation CSV files
    logger.info("Saving annotation data...")
    save_annotations()
    
    # Clean up old logs before closing
    cleanup_old_logs()
    
    logger.info("All data saved successfully, program closing")
    print("✓ All data saved successfully!")
    print("✓ Program closing...")

# Old return_to_welcome function removed - now using modular approach

# Old event connection code removed

# Old button connection code removed

# Old image button connection code removed

# Old print status code removed

# Final safety check
try:
    pass  # Placeholder for removed initialization code
except Exception as e:
...
def process_csv_file(file_path):
    ...
    try:
        ...
        with open(test_file, 'w') as f:
            f.write('test')
        os.remove(test_file)
        logger.info(f"Output directory created and verified writable: {output_dir}")
        print(f"✓ Output directory created and verified writable: {output_dir}")
    except Exception as e:
        logger.error(f"Error creating/verifying output directory: {e}")
        ...
        try:
            ...
            logger.critical(f"Critical error: Cannot create output directory: {e2}")
...
if __name__ == "__main__":
    ...
    # If imported as a module, just show welcome screen once
    try:
        file_path = show_welcome_screen_and_get_filepath()
        if file_path:
            process_csv_file(file_path)
